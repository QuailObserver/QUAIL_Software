...
#	pragma pack(pop)
	...
	public:
         ...
         *  \param bufferSize a reference to a variable which passes in the size of the supplied buffer, and updates with the length of the string returned.
		 *
	...
	*/
	...
	public:
		...
		enum Model {
			...
			RESERVEDC1 = 0x200,	//!< Reserved for subvariant 2
			RESERVEDD1 = 0x300,	//!< Reserved for subvariant 3
			RESERVEDE1 = 0x400,	//!< Reserved for subvariant 4
		...
		enum Status
		{
			Idle,            //!< Sensor is in idle state, and can accept ISensor::startExposure() commands. 0
			Trigger,         //!< Sensor is waiting for trigger. 1
			PreShutter,      //!< Sensor is preparing for shutter movement. 2
			DoShutterOpen,   //!< Sensor is opening the shutter. 3
			Starting,        //!< Sensor is starting the exposure. 4
			Exposing,        //!< Sensor is exposing. 5
			DoShutterClose,  //!< Sensor is closing the shutter post-exposure. 6 
			Reading,         //!< Sensor is reading data to buffer. 7
			ReadyToDownload, //!< Sensor is ready to transfer the image. 8 
			HomingShutter,	 //!< Sensor is waiting for the shutter to return home. Sensor is busy, and is unable to start an exposure until the shutter is homed. 9
			InvalidSensorState = 0xFF, //!< The sensor status is uninitialized. 10
		};
...
#		pragma pack(pop)
         ...
         * \param key key the key value to retrieve
         ...
         *  \param params the parameters of the exposure.
		 *  \return IPromisePtr pointer to promise for monitoring the status of the asynchronous operation.
...
#if defined(_WIN32) || defined(_WIN64)
	typedef IGatewayPtr(MYCDECL * f_GetGateway)();
	typedef void(MYCDECL * f_DeleteGateway)(IGatewayPtr);	
#endif
